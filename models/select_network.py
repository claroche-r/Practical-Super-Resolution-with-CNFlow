...
def define_G(opt):
    ...
    if net_type == 'cnflow':
        ...
        netG = net(opt_net['in_nc'],
                   opt_net['n_flow'],
                   n_blocks=opt_net['nb'],
                   actnorm_scale=opt_net['actnorm_scale'],
                   LU_decomposed=opt_net['LU_decomposed'],
                   opt=opt_net)

    elif net_type == 'Predictor':
        from models.network_kernelpred import Predictor as net
        netG = net(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'])

    elif net_type == 'rrdb':
        from models.network_sr import RRDB as net
...
def define_D(opt):
    ...
    elif net_type == 'patch_gan':
        from models.network_discriminator import PatchGAN as discriminator
        netD = discriminator(opt_net['in_nc'], ndf=opt_net['base_nc'])
