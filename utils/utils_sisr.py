...
def build_bicubic_filter(factor):
    size = factor * 4 + 1
    k = np.zeros(size)
    x = [-2 + i * (4 / (size + 1)) for i in range(1, size + 1)]
    for i, elem in enumerate(x):
        k[i] = bicubic_kernel(elem)
    k = k / np.sum(k)
    # make 2d
    k = np.outer(k, k.T)
    return k
...
def extend_kernel_size(kernel, size):
    h, w = kernel.shape
...
def kernel_shift(kernel, sf=4):
    ...
    current_center_of_mass = ndimage.measurements.center_of_mass(kernel)
    ...
    kernel = ndimage.interpolation.shift(kernel, shift_vec)
